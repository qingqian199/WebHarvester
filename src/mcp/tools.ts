...
export function registerMcpTools(server: McpServer, ctx: ToolContext): void {
  ...
  server.registerTool({
    ...
    handler: async (args) => {
      ...
      const storage = new CompositeStorageAdapter([new FileStorageAdapter(appCfg.outputDir), new SqliteStorageAdapter()]);
  ...
  server.registerTool({
    ...
    handler: async (args) => {
      ...
      // 搜索
      const searchUnit = site === "xiaohongshu" ? "search_notes" : site === "zhihu" ? "zhihu_search" : site === "bilibili" ? "bili_search" : "";
        ...
        for (const dir of dirs) {
          ...
          for (const f of files) {
            if (
              !f.endsWith(".json") ||
              f.endsWith("-api.csv") ||
              f.endsWith("-anti-crawl.json") ||
              f.endsWith("-wbi-test.py") ||
              f.endsWith("-wbi-stub.py")
            )
              continue;
  ...
  server.registerTool({
    ...
    description:
      "从 CDP 连接的 Chrome 浏览器中提取所有目标站点的 Cookie 并写入本地会话文件。支持站点：bilibili / xiaohongshu / zhihu / xueshu。merge=true 时以合并模式写入（保留已有 Cookie 中未出现在新数据中的条目，如小红书 a1）。",
    inputSchema: {
      type: "object",
      properties: {
        merge: {
  ...
  // Diagnostics & monitoring tools (registered in tools-diagnostics.ts)
