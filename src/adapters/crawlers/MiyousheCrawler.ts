...
import { buildMiyousheHeaders } from "../../utils/crypto/miyoushe-signer";
import { buildCookieHeader } from "../../utils/cookie-helper";
import { UnitResult } from "../../core/models/ContentUnit";
import { BaseCrawler } from "./BaseCrawler";
import type { MiyoushePostDetail, MiyousheUserInfo, MiyousheCommentItem, MiyousheSearchItem } from "../../core/models/crawler-data";
...
export class MiyousheCrawler extends BaseCrawler {
  ...
  private registerPostDetailHandler(): void {
    this.unitHandlers.set("miyoushe_post_detail", async (unit, params, session) => {
      ...
      try {
        ...
        if (data.retcode === 0) {
          const postData = ((data.data as Record<string, unknown>)?.post as Record<string, unknown>) || {};
          const post = (postData.post as Record<string, unknown>) || {};
          const stat = (postData.stat as Record<string, unknown>) || {};
          const imgs = (postData.image_list as Array<Record<string, unknown>>) || [];
          const images = imgs.map((img) => String(img.url || img.src || "")).filter(Boolean);
          const topics = (postData.topics as Array<Record<string, unknown>>) || [];
          return {
            unit,
            status: "success",
            data: {
              ...
              plainText: String(post.content || "")
                .replace(/<[^>]+>/g, "")
                .trim(),
              images,
              uid: String(post.uid || ""),
              created_at: this.safeNum(post.created_at),
              view_status: this.safeNum(post.view_status),
              is_original: this.safeNum(post.is_original),
              forum_id: this.safeNum(post.f_forum_id),
              topics: topics.map((t) => String(t.name || "")).filter(Boolean),
              stats: {
                view: this.safeNum(stat.view_num),
                like: this.safeNum(stat.like_num),
                reply: this.safeNum(stat.reply_num),
                favorite: this.safeNum(stat.bookmark_num),
                share: this.safeNum(stat.share_num || stat.forward_num),
              },
            } as MiyoushePostDetail,
            method: "signature",
            responseTime,
          };
        }
        return {
          ...
          method: "signature",
          responseTime,
          error: `米游社 API 返回异常: retcode=${data.retcode}`,
        };
      } catch (e: unknown) {
        ...
        if (data.retcode === 0) {
          ...
          return {
            ...
            data: {
              ...
              introduce: String(info.introduce || ""),
              avatar_url: String(info.avatar_url || ""),
              gender: this.safeNum(info.gender),
              level: this.safeNum(levelExp.level),
              level_exp: this.safeNum(levelExp.exp),
              like_num: this.safeNum(achieve.like_num),
              post_num: this.safeNum(achieve.post_num),
              replypost_num: this.safeNum(achieve.replypost_num),
              follow_cnt: this.safeNum(achieve.follow_cnt),
              followed_cnt: this.safeNum(achieve.followed_cnt),
              is_realname: !!community.is_realname,
              ip_region: String(data.data ? ((data.data as Record<string, unknown>).user_info as Record<string, unknown>)?.ip_region || "" : ""),
            } as MiyousheUserInfo,
            method: "signature",
            responseTime,
          };
          ...
          const items: MiyousheCommentItem[] = list.map((r) => {
            ...
            return {
              ...
              content: String(reply.content || "")
                .replace(/<[^>]+>/g, "")
                .trim(),
              like_count: this.safeNum(reply.like_count),
              created_at: this.safeNum(reply.created_at),
              sub_reply_count: this.safeNum(reply.sub_reply_count || (reply as any).r_rcount || 0),
            };
          });
  ...
  private registerSearchPostsHandler(): void {
    this.unitHandlers.set("miyoushe_search_posts", async (unit, params, session) => {
      ...
      try {
        const { responseTime, data } = await this.signedGet(url, query, session);
        if (data.retcode === 0) {
          const rawList = (data.data as Record<string, unknown>)?.posts || (data.data as Record<string, unknown>)?.list || [];
          ...
          const items: MiyousheSearchItem[] = list.map((p) => {
            ...
            return {
              post_id: String(post.post_id || p.post_id || ""),
              subject: String(post.subject || ""),
              uid: String(post.uid || user.uid || ""),
              nickname: String(user.nickname || ""),
              forum_name: String(forum.name || ""),
              created_at: this.safeNum(post.created_at),
              reply_count: this.safeNum(stat.reply_count || stat.reply || 0),
              like_count: this.safeNum(stat.like_count || stat.like || 0),
              view_status: this.safeNum(post.view_status),
            };
          });
          return { unit, status: items.length > 0 ? "success" : "partial", data: items, method: "signature", responseTime };
  ...
  async collectUnits(units: string[], params: Record<string, string>, session?: CrawlerSession): Promise<UnitResult<unknown>[]> {
    // 并行执行所有单元（米游社单元间无依赖）
    const tasks = units.map(async (unit) => {
      const start = Date.now();
      try {
        return await this.dispatchUnit(unit, params, session);
      } catch (e: unknown) {
        return {
          unit,
  ...
  private async signedGet(
  ...
  ): Promise<{ r: PageData; responseTime: number; data: Record<string, unknown> }> {
    ...
    const data = JSON.parse(await _r.text()) as Record<string, unknown>;
    return {
      r: {
        url: _r.url,
        statusCode: _r.status,
        body: JSON.stringify(data),
        headers: Object.fromEntries(_r.headers),
        responseTime,
        capturedAt: new Date().toISOString(),
      },
      responseTime,
