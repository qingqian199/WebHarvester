...
export class PlaywrightAdapter implements IBrowserAdapter {
  /** @internal 公开以供 BaiduScholarCrawler 等需要直接操作 LCM 的场景使用 */
  get lcm(): BrowserLifecycleManager {
    return this._lcm;
  }
  private _lcm: BrowserLifecycleManager;
  private poolSite = "";
  /** 是否通过 CDP 连接到已有 Chrome 实例（跳过 launch）。 */
  private _cdpAttached = false;

  constructor(private readonly logger: ILogger) {
    this._lcm = new BrowserLifecycleManager(logger);
  }
  ...
  async launch(
    ...
    if (this._cdpAttached) {
      if (enableFullCapture || captureAllTypes) {
        this._lcm.disableNetworkCapture();
        this._lcm.startNetworkCapture(enableFullCapture, captureAllTypes);
      }
      const extraWait = enableFullCapture || captureAllTypes ? REQUEST_CAPTURE_EXTRA_WAIT_MS : 1000;
      await new Promise((r) => setTimeout(r, extraWait));
      return;
    }
    await this._lcm.launch(url, true, sessionState, "domcontentloaded", undefined, proxyUrl, pageSetup, enableFullCapture, captureAllTypes);
  }

  /** 连接到 ChromeService 管理的已有 Chrome 实例 (CDP)。返回新的 PlaywrightAdapter。 */
  static async connectToChromeService(
    port: number,
    url: string,
    logger: ILogger,
    sessionState?: SessionState,
  ...
  private _injectCdpBrowser(browser: any): void {
    this._lcm.setBrowser(browser);
  }

  /** 获取当前 LCM 管理的 Page 实例。 */
  getPage(): import("playwright").Page | null {
    try {
      return this._lcm.getPage();
    } catch {
      return null;
    }
  }

  /** 替换内部的 BrowserLifecycleManager（用于自定义启动流程后复用适配器）。 */
  replaceLifecycleManager(lcm: BrowserLifecycleManager): void {
    this._lcm = lcm;
  }

  /** 设置浏览器页面实例（用于 BaiduScholarCrawler 等自定义启动流程）。 */
  setPage(page: import("playwright").Page): void {
    this._lcm.setPage(page);
  }
  ...
  async attachToContext(
    ...
    this.poolSite = site || "";
    await this._lcm.attachToContext(context, url, sessionState, pageSetup);
  }

  async performActions(actions: HarvestConfig["actions"]): Promise<void> {
    if (!actions?.length) return;
    const page = this._lcm.getPage();
  ...
  async captureNetworkRequests(config: { captureAll: boolean; enhancedFullCapture?: boolean }): Promise<NetworkRequest[]> {
    const page = this._lcm.getPage();
    ...
    if (config.enhancedFullCapture) {
      try {
        ...
        for (const sel of ["text=加载更多", "text=展开回复", "text=查看更多回复", ".load-more", ".comment-expand"]) {
          try {
            const btns = await page.$$(sel);
            for (const btn of btns) {
              const box = await btn.boundingBox();
    ...
    return this._lcm.getCapturedRequests();
  }
  ...
  getPageMetrics() {
    return this._lcm.getPageMetrics();
  }
  ...
  getPageDiagnostics(): { consoleMessages: Array<{ type: string; text: string }>; pageErrors: Array<{ message: string; stack?: string }> } {
    return this._lcm.getPageDiagnostics();
  }

  async close(): Promise<void> {
    await this._lcm.close();
