...
export abstract class BaseCrawler implements ISiteCrawler {
  ...
  async fetch(url: string, session?: CrawlerSession, options?: FetchOptions): Promise<PageData> {
    ...
    const result = await this.pipeline.execute(ctx, async (c: CrawlContext) => {
      try {
        ...
        const res = await fetch(c.url, fetchInit);
  ...
  protected async extractSSRData(browser: PlaywrightAdapter, degradedFrom = "Signature", errorCode?: number): Promise<{ body: string }> {
    ...
    const bodyText: string = await browser.executeScript<string>("document.body.innerText.slice(0, 10000)").catch(() => "");
  ...
  protected async genericFetchPageData(
    ...
    if (waitSelector) {
      try {
        await browser.getPage()?.waitForSelector(waitSelector, { timeout: 8000 });
      } catch {} // ok: ignored
    } else {
  ...
  protected async fetchPageContent(
    ...
    try {
      ...
      await adapter.attachToContext(context, url, sessionState, pageSetup, siteKey);
    } catch (e) {
    ...
    if (contentSelector) {
      try {
        await adapter.getPage()?.waitForSelector(contentSelector, { timeout: 8000 });
      } catch {} // ok: ignored
    } else {
  ...
  /** 策略映射表，子类在构造器中注册处理函数。 */
  protected unitHandlers: Map<
    string,
    (unit: string, params: Record<string, string>, session?: CrawlerSession, authMode?: string, results?: UnitResult[]) => Promise<UnitResult>
  > = new Map();

  /** 从策略映射中查找并执行处理函数，未找到返回 failed。 */
  protected async dispatchUnit(
    unit: string,
  ...
  protected async traverseSubReplies<T>(rootItems: T[], options: SubReplyTraverseOptions): Promise<SubReplyResult> {
    ...
    const rpidResults: (RpidResult | null)[] = await this.runWithConcurrency(
      target,
      concurrency,
      async (rootId: string): Promise<RpidResult | null> => {
        try {
          ...
          for (let p = 0; p < maxPages; p++) {
            ...
            if (result.replies.length > 0) {
              allReplies = allReplies.concat(result.replies);
          ...
          if (options.postProcess) {
            const pp = options.postProcess(processedReplies, rootId);
            processedReplies = pp.replies;
          }
          return { rootId, replies: processedReplies, all_count: processedReplies.length, subReplyTime: totalTime };
        } catch (e: unknown) {
          this.logger.warn(`子回复遍历失败 (rootId=${rootId}): ${e instanceof Error ? e.message : String(e)}`);
          return null;
        }
      },
    );
    ...
    for (const rr of rpidResults) {
      ...
      if (rr.all_count > 0) {
        byRpid[rr.rootId] = { replies: rr.replies, all_count: rr.all_count };
        totalReplies += rr.all_count;
        expandedCount++;
      }
